...
<div class="rounded-lg dark:border-gray-700">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-2.5 w-full mb-4">
        <div class="flex justify-center lg:justify-start w-full">
            <h1 class="flex self-center text-2xl font-extrabold dark:text-white uppercase">Data Fakultas</h1>
        </div>

        <!-- Button with Modal -->
        <div class="flex justify-center lg:justify-end items-center gap-1.5 w-full">
            <form method="get" action="{{ route('admin.cari-fakultas') }}" class="flex">
                <input type="text" id="cari" name="cari" placeholder="cari nama fakultas" class="bg-gray-50 border border-black text-gray-900 text-xs rounded-s-lg focus:ring-blue-500 focus:border-blue-500 block w-60 h-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        ...
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 border border-black">
                ...
                <div class="px-6 py-6 lg:px-8">
                    ...
                    <form class="space-y-6" method="POST" action="{{ route('admin.create-fakultas') }}">
                        @csrf
                        <div>
                            <label for="nama_fakultas" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                Fakultas</label>
                            <input type="text" id="nama_fakultas" name="nama_fakultas" placeholder="fakultas" class="bg-gray-50 border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                        </div>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-2.5 py-2 text-right flex justify-end">
                        <form method="POST" action="{{ route('admin.edit-fakultas', $data->id) }}">
                            @csrf
                            <button type="submit" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-4 bg-none">
                                Edit
                            </button>
                        </form>
                        <form method="POST" action="{{ route('admin.delete-fakultas', $data->id) }}">
                            @csrf
                            @method('DELETE')
    ...
    <div class="flex justify-end items-center mt-4 gap-1.5 w-full font-medium text-xs">
        ...
        <div class="flex items-center w-fit">
            <a href="{{ $fakultas->previousPageUrl() }}" title="Sebelumnya" class="flex h-[30px] p-1.5 self-center text-white bg-black hover:bg-gray-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-s-lg dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 cursor-pointer">
                <svg class="w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
                    <path d="M560-280 360-480l200-200v400Z" />
                </svg>
            </a>

            <a href="{{ $fakultas->nextPageUrl() }}" title="Selanjutnya" class="flex h-[30px] p-1.5 self-center text-white bg-black hover:bg-gray-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-e-lg dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 cursor-pointer">
                <svg class="w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
                    <path d="M400-280v-400l200 200-200 200Z" />
                </svg>
            </a>
