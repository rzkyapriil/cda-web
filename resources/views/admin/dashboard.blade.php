...
@section('main')
  ...
  <div class="flex flex-col gap-3 items-center justify-center">
    <div
      class="flex items-center justify-center text-sm font-bold w-full py-2.5 rounded-lg border border-black bg-white uppercase">
      Rekap Penilaian
    </div>
    <div class="flex flex-col lg:flex-row justify-center items-center gap-2 text-xs w-full uppercase">
      <div class="hidden md:block w-full text-nowrap text-sm font-bold">
        Filter Data
      </div>
      ...
      <form method="get" action="{{ route('admin.filter-dashboard') }}"
        ...
        <div class="flex w-full">
          ...
          <div class="w-fit">
            <svg class="h-8 fill-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
        ...
        <div class="w-full lg:w-fit">
          <button type="submit" id="start-process"
            class="px-4 text-sm font-bold bg-black hover:bg-gray-300 text-gray-50 hover:text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            Filter
          </button>
        </div>
      </form>
      ...
      <form method="get" action="{{ route('admin.export-dashboard') }}" class="flex w-full lg:w-fit">
        ...
        <button type="submit" id="export-process"
          class="px-4 text-sm font-bold bg-green-500 hover:bg-green-600 text-gray-50 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          <div id="export-text">Export</div>
          <div id="export-load" class="hidden justify-center items-center">
            <lord-icon src="https://cdn.lordicon.com/gkryirhd.json" colors="primary:#ffffff" trigger="loop"
              state="loop-snake-alt" class="w-5 h-5"></lord-icon>
          </div>
        </button>
      </form>
    </div>

    <div id="status" class="flex flex-col gap-3 justify-center items-center w-full h-96">
      <lord-icon src="https://cdn.lordicon.com/gkryirhd.json" trigger="loop" state="loop-snake-alt" class="w-8 h-8">
      </lord-icon>
      <div id="status-text" class="text-sm"></div>
    </div>

    <div class="relative overflow-x-auto shadow-md w-full" id="tabel_area">
        ...
        <thead class="text-black uppercase bg-white dark:bg-gray-700 dark:text-gray-400 mb-2.5 border border-black">
          ...
          <tr class="text-center">
            @for ($i = 1; $i <= $jumlah_nilai; $i++)
              <th scope="col" class="px-2.5 py-1.5 border-e border-b border-black">
                {{ $i }}
              </th>
            @endfor
    ...
    <div class="relative overflow-x-auto shadow-md w-full" id="tabel_fakultas">
      <table class="w-full text-xs text-left text-gray-500 dark:text-gray-400">
        <thead class="text-black uppercase bg-white dark:bg-gray-700 dark:text-gray-400 mb-2.5 border border-black">
          <tr class="text-center rounded-lg">
            <th scope="col" rowspan="2" class="px-2.5 py-2 border-e border-black">
    ...
    <div class="relative overflow-x-auto shadow-md w-full" id="tabel_jurusan">
      <table class="w-full text-xs text-left text-gray-500 dark:text-gray-400">
        <thead class="text-black uppercase bg-white dark:bg-gray-700 dark:text-gray-400 mb-2.5 border border-black">
          <tr class="text-center rounded-lg">
            <th scope="col" rowspan="2" class="px-2.5 py-2 border-e border-black">
      ...
      $('#start-process').click(function() {
        $.ajax({
          ...
          beforeSend: function() {
            ...
            $('#tabel_fakultas').hide();
            $('#tabel_jurusan').hide();
            $('#status-text').text('Tunggu sebentar . . . ');
          },
